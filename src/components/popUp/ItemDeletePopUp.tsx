...
import { useDeleteItem } from "@/hooks/useAPI";
import { toast } from "sonner";
const ItemDeletePopUp = ({ open, onOpenChange, item }: { open: boolean, onOpenChange: (open: boolean) => void, item: any | null }) => {
            ...
            <AlertDialogPortal>
                ...
                <AlertDialogContent
                    ...
                    <AlertDialogHeader style={{ padding: '0.5rem 1rem', borderRadius: '0.5rem' }} className="text-center">
                        <AlertDialogTitle className="text-2xl font-bold">Delete Item</AlertDialogTitle>
                    </AlertDialogHeader>
                    <AlertDialogDescription style={{ padding: '0.5rem 1rem', borderRadius: '0.5rem' }} className="text-sm text-muted-foreground space-y-2">
                        <p>Are you sure you want to delete <span className="font-bold">{item?.name}</span>?</p>
